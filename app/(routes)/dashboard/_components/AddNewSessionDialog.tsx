...
import { ArrowRight, CloudHail, Loader2 } from "lucide-react";
...
function AddNewSessionDialog() {
  ...
  return (
    <Dialog onOpenChange={(open) => {
        if (!open) {
          // Dialog is closing, reset all relevant states
          setSuggestedDoctors([]);
        }
      }}>
      ...
      <DialogContent>
        ...
        <DialogFooter>
          <DialogClose  asChild >
            <Button variant={"outline"} onClick={()=>{setSuggestedDoctors([])}}>Cancel</Button>
          </DialogClose>
          {suggestedDoctors.length==0? (
